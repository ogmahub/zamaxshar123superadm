        ...
        <div className="fixed inset-0 bg-black/50 z-50 grid place-items-center p-4" onClick={() => setEditing(null)}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={submit} className="card p-6 w-full max-w-3xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              <div><label className="label block mb-1">Nom (UZ)</label><input className="input" required value={form.titleUz} onChange={(e) => setForm({ ...form, titleUz: e.target.value })} /></div>
              <div><label className="label block mb-1">Nom (RU)</label><input className="input" value={form.titleRu} onChange={(e) => setForm({ ...form, titleRu: e.target.value })} /></div>
              <div><label className="label block mb-1">Nom (EN)</label><input className="input" value={form.titleEn} onChange={(e) => setForm({ ...form, titleEn: e.target.value })} /></div>
              <div className="sm:col-span-3"><label className="label block mb-1">Tavsif (UZ)</label><textarea className="input" rows={2} value={form.descriptionUz} onChange={(e) => setForm({ ...form, descriptionUz: e.target.value })} /></div>
              <div className="sm:col-span-3"><label className="label block mb-1">Tavsif (RU)</label><textarea className="input" rows={2} value={form.descriptionRu} onChange={(e) => setForm({ ...form, descriptionRu: e.target.value })} /></div>
              <div className="sm:col-span-3"><label className="label block mb-1">Tavsif (EN)</label><textarea className="input" rows={2} value={form.descriptionEn} onChange={(e) => setForm({ ...form, descriptionEn: e.target.value })} /></div>
              ...
              <div className="sm:col-span-3 flex items-center gap-2"><input type="checkbox" checked={form.isActive} onChange={(e) => setForm({ ...form, isActive: e.target.checked })} /><label>Active</label></div>
            </div>
